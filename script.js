...
for (const operator of operators) {
  operator.addEventListener("click", function () {
    ...
    if (end) {
      // operator is pressed after result
      operatorInput = input;
      a = storeResult;
      ...
      // operator is pressed after num op num
      operatorInput = input;
...
dot.addEventListener("click", function () {
  if (!dotPressed) {
    // if dot is not pressed
    numberSequence += ".";
    console.log(numberSequence[numberSequence.length - 1]);
    display.innerHTML = numberSequence;
    dotPressed = true;
  }
});
...
equals.addEventListener("click", function () {
  if (operatorInput == "") {
    // if = is pressed after an operator, do nothing
    // do nothing
  } else {
    // compute result only if there is an operator input
    dotPressed = false;
    let result = operate(operatorInput, a, numberSequence);
    storeResult = result;
    console.log(result);
